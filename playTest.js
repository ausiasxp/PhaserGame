...
function initialiseGame() {
    ...
    //Minijuego palabras
    game.input.keyboard.addCallbacks(this, null, null, keyPress); //pillamos input de teclado para llamar a keyPress

    //inicializamos variables
    wordFound = false;
    word = "";

    let palabraElegida = Math.random(8);
    currentWord = misPalabras[palabraElegida];
    text = game.add.text(30,30, "", {fill:'#ffffff'});
    winText = game.add.text(30,60, wordFound, {fill:'#ffffff'});
}
...
function gameUpdate() {
    ...
    if (spaceKey.isDown && player.body.touching.down && hitPlatform) {
...
function keyPress(char){
    ...
    if (wordFound == false){
        if (word.length < currentWord.length){
            word += char;
            text = game.add.text(30,30, word, {fill:'#ffffff'});
        }
        else {
            if (word == currentWord){
                wordFound = true;
